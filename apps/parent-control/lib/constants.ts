...
// (arch.md §896, issue #147): `knowledge:<ns>`. It is a SIGNED cap field — the
// broker hashes it (`keccak`) for `isServiceInScope`, the worker keys storage
// off it (`bots/<actor>/memory/knowledge:<ns>.enc`), and the grant must match
// exactly. A bare `memory` never matches a `knowledge:<ns>` grant →
...
export function knowledgeService(ns: string): string {
  return `knowledge:${ns}`;
}
