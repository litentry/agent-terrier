#!/usr/bin/env bash
# scripts/utils/check-wallet-balances.sh — read-only HEI balance snapshot for the
...
#
# Usage:
#   bash scripts/utils/check-wallet-balances.sh                 # prod profile
#   CHAIN_PROFILE=crates/agentkeys-core/chain-profiles/heima-paseo.json bash scripts/utils/check-wallet-balances.sh
set -euo pipefail
