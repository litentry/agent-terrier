...
/v1/sandbox/self/actions	2026-09-24 plan dsh-plugin-abstraction PR 1 — the ADVERTISED action list the dsh suite registers as tools; SANDBOX-INTERNAL (delegate daemon, sandbox-self bearer), never a web surface. Pure over env: pinned by agentkeys-protocol sandbox_actions::tests against e2e/fixtures/bridge-protocol/actions_contract.json (the suite's actions.spec.ts reads the same fixture) + daemon actions::tests. Retire with a live delegate step (a spawned dsh sandbox on the test stack).
/v1/sandbox/self/publish	2026-09-24 the advertised publish_to_slot verb — SANDBOX-INTERNAL; the same SelfBackend::publish path as `--publish-once`, driven end-to-end by the image smoke (docker/dsh-sandbox/smoke/smoke.sh step 2b against the fake sink); body→PublishInput pinned by ui_bridge::sandbox_actions_tests. Retire with a live delegate publish step.
/v1/sandbox/self/propose	#573 / spec §4.4 the advertised propose_to_owner verb AND the answerer's runtime-ask sink — SANDBOX-INTERNAL; the same SelfBackend::propose path as `--propose-once` (smoke.sh steps 2c + 2e); body→ProposalInput pinned by ui_bridge::sandbox_actions_tests. Retire with a live delegate runtime-ask step.
...
/v1/master/agent/preset/reapply	2026-09-17 preset re-apply (persona + skills into the LIVE sandbox) — needs a spawned delegate with a reachable bridge (pair with the spawn waiver); the readiness gate + non-JSON decode are unit-tested. Retire when a suite-6 step drives a live delegate's context apply.
