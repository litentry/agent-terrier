#!/usr/bin/env bash
# scripts/check-broker-routes-fresh.sh — fail loud when the shared test broker
...
if [ "$missing" -gt 0 ] || [ "$unreachable" -gt 0 ]; then
  ...
  if [ "$missing" -gt 0 ]; then
    ...
    echo "xx    bash scripts/setup-broker-host.sh --ci --ref <this-branch-or-main>   (on the test broker host)" >&2
  fi
