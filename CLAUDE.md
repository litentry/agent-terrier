...
**Flag convention (all three):** run **PLAIN for local/prod**; add **`--ci`** (alias `--test`) for the CI/test environment. `--ci` selects the CI broker EIP (tag `agentkeys-broker-eip-test`), `-test`-suffixed IAM identifiers + buckets, and the `*.test.env` files. The test fleet (#265) adds ONE more dimension: **`--ci --slot N`** (or `AGENTKEYS_TEST_SLOT=N`; auto-detected from a `*test-N*` env-file path) selects test-fleet slot N — `-test-N` identifiers, EIP tag `agentkeys-broker-eip-test-N`, `*.test-N.env` files. A bare `--ci` = slot 1 (grandfathered plain `-test` names; its OIDC issuer `test-broker.${ZONE}` is byte-frozen — see [`docs/cloud-bootstrap.md`](docs/cloud-bootstrap.md) §0.3). Never hand operators a pile of env flags — **plain = prod, `--ci` = CI, `--ci --slot N` = fleet slot N**. (Per-step targeting — `--only-step N`, `--ref main`, `--reclaim-toolchain` — is fine; *environment* selection is `--ci [--slot N]` or nothing.) On the **broker host**, `--ci`/`--slot` matter only for the FIRST bootstrap of a virgin machine: once the broker unit exists, `setup-broker-host.sh` re-runs self-identify the environment + slot from the deployed unit's `BROKER_OIDC_ISSUER` (a contradicting explicit `--slot` is a hard error — cross-wiring guard).
...
- **By tag, keyed on env + slot** (canonical — `setup-cloud.sh` step 4 + `dns-upsert-workers.sh`): `aws ec2 describe-addresses --region "$REGION" --filters Name=tag:Name,Values=agentkeys-broker-eip[-test[-N]] --query 'Addresses[0].PublicIp'`. Both scripts pick the `-test` suffix from `TEST_MODE` (`--test`, or a `*test*` ENV_FILE) and the `-N` slot suffix from `--slot N` / `AGENTKEYS_TEST_SLOT` / a `*test-N*` ENV_FILE.
...
|---|---|---|---|
| **1. Broker cap-mint** | The session JWT's `agentkeys.omni_account` claim MUST match the request's `operator_omni`. Also: `device.operator_omni == session_omni`, `device.actor_omni == req.actor_omni`, `device.roles & ROLE_CAP_MINT`, `isServiceInScope(operator, actor, service) == true`, **and — when supplied — a valid K10 cap proof-of-possession** (`verify_cap_pop`: `client_sig` recovers to an address whose `keccak == device_key_hash`, fresh — issue #76). Returns `OperatorMismatch` / `DeviceBindingMismatch` / `DeviceRoleMissing` / `ServiceNotInScope` / `CapPopInvalid` otherwise. | [`handlers/cap.rs`](crates/agentkeys-broker-server/src/handlers/cap.rs) — `mint_cap()` + `verify_cap_pop()` | [`harness/v2-stage3-demo.sh`](harness/v2-stage3-demo.sh) step 13 (NEGATIVE cap-mint with cross-actor `operator_omni` → HTTP 4xx) |
| **2. Worker chain-verify** | Independent re-check of layer-1 invariants from the worker's perspective — defense-in-depth against broker compromise. `verify_signature` (broker cap-sig), **`enforce_client_pop` (K10 proof-of-possession, re-verified independently of the broker: a *supplied* `client_sig` is always validated; a *missing* one is REJECTED only under `AGENTKEYS_WORKER_REQUIRE_CAP_POP=1` — default OFF during the staged rollout, flip after every actor's K10 incl. the master's is registered, then a compromised broker cannot mint a usable cap; issue #76)**, `check_chain_device`, `check_chain_scope`, `check_chain_k3_epoch`. | [`crates/agentkeys-worker-creds/src/verify.rs`](crates/agentkeys-worker-creds/src/verify.rs) (shared by the cred/memory/config/classify workers) | [`harness/v2-stage3-demo.sh`](harness/v2-stage3-demo.sh) steps 11+12 (full HTTP roundtrip exercises every verify hook) |
