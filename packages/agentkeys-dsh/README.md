...
| `@agentkeys/dsh-suite/propose` | `agentkeys-propose` | the `propose_to_owner` tool — the delegate's "propose" verb: one durable learning into the owner's review queue via `agentkeys-daemon --propose-once` (rate- and size-bounded there), no shell — the image's `propose-to-owner` helper needs `tool:code`, which no app sheet grants, so no application could propose. Guard verdict `propose`: allowed by ANY `proposal:<ns>` grant (every installed application holds its own inbox), the namespace itself is the cap-mint's verdict |
