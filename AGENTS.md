...
- [`design-system/`](design-system/) — shared `@agentkeys/design-system`: design tokens (3 brand themes + the parent-control console's neutral `github` surface, which also overrides fonts / neutrals / radii; bilingual EN/中文) + React components, consumed by every app (`tokens.json` is the color source-of-truth → `scripts/generate-tokens.mjs`).
...
**User-facing instructions** — every behavior/caveat a user would notice (e.g. the agent's OpenViking memory being bounded by the user's grants, #566) goes in [`docs/user-manual.md`](docs/user-manual.md), the single home for user-aware instructions.
...
## Design-rules gate (arch.md §1a)
[arch.md §1a "Design rules"](docs/arch.md) is **normative**: D1 chain-is-the-single-source-of-authority · D2 stateless broker · D3 keys-never-leave-their-machine · D4 no ambient authority · D5 broker-never-writes-chain · D6 PII off-chain · D7 one-owner-per-contract. **Before any change that adds a durable store, puts a secret/credential at rest, adds or bypasses an authorization check, or introduces a new cross-component contract, check it against that table.** (D4 draws the user-vs-stack line: a *user* credential never rests anywhere; a *stack* credential — a cloud identity or edge token the deployment owns — may rest host-local ONLY when a setup entry point provisions it, per the `AGENTS.ops.md` wire-in rule. A hand-placed stack secret needs no registry row because it is simply not allowed.) If the change bends a rule, it ships ONLY with a row in the §1a **exception registry** (scope + bound, why, revert path) in the SAME PR — never as prose buried in a feature section, and never silently. Code that violates a rule with no registry row IS the bug: fix the code, or register the exception explicitly and get it reviewed. (Why this gate exists: #546 first shipped a durable broker store — with a delegate key at rest — documented only inside a feature paragraph while §13/§14.3 still said "stateless"; the contradiction was caught in review, not by any doc or gate.)
...
**Branch-naming rule (owner decision, 2026-07-16): the PR branch is named after the worktree.** Claude Code mints the worktree name and the branch name independently (two unrelated random codenames — e.g. a real session left worktree `nifty-yalow-6e5c6f` holding branch `claude/focused-jones-95962b`), so by default an operator looking at a worktree directory cannot tell which branch/PR it holds without running git. Therefore: the FIRST branch created from `.claude/worktrees/<name>` MUST be `claude/<name>`; a second or later branch cut from the same worktree is numbered `claude/<name>-2`, `claude/<name>-3`, … Never accept an auto-generated branch name that diverges from the worktree; rename before the first push (`git branch -m claude/<name>`). (Renaming a branch that already has an open PR breaks the PR — the rule applies from branch creation, not retroactively.)
...
## Worker URLs are DERIVED, never a pre-composed host env

**A worker's public URL = `https://<worker-name>.<zone>`, inferred from the worker name + the broker's domain — NEVER a stack-specific pre-composed host env var** (`VE_CHANNEL_HOST`, `VE_MEMORY_HOST`, …). The ONE owner of this derivation is [`ui_bridge::derive_worker_url(broker_url, worker)`](crates/agentkeys-daemon/src/ui_bridge.rs) — it extracts the zone from the session broker host and prepends the worker name, handling the per-stack prefix (`broker.` → ``, `test-broker.` → `-test`, `broker-<x>.` → `-<x>`), so `derive_worker_url("https://broker.agentterrier.cn", "channel")` = `https://channel.agentterrier.cn`. Every consumer resolves a worker URL the SAME way: **an explicit `AGENTKEYS_WORKER_<X>_URL` override if set, ELSE derive** (`channel_worker_url`, the audit/weixin URLs, and the in-sandbox `chat_loop::from_env` all follow this).

**Why:** a pre-composed `VE_CHANNEL_HOST`-style env is a second source of truth that drifts from the broker's own domain and forces every spawn/caller to remember to inject it (the #543 "no agent reply" bug: the chat loop *required* an injected `AGENTKEYS_CHANNEL_WORKER_URL` the VE broker never set, so it never started). Deriving from the broker host — which every actor already holds — means a component that knows its broker automatically knows every co-located worker, with zero extra wiring.

**Rules:**
- **Code:** call `derive_worker_url(broker, "<worker>")`; treat any `AGENTKEYS_WORKER_<X>_URL` as an *optional override*, never a required input. Never hand-compose `format!("https://{host}", host = some_env)` from a pre-baked host var.
- **Scripts / units:** when a URL genuinely must be rendered into an env (e.g. a legacy sandbox image predating the in-code derivation), compose it from the worker name + the zone (`https://<worker>.${zone}`, `zone="${VE_CN_ZONE:-${VE_BROKER_HOST#*.}}"`), NOT `https://${VE_<X>_HOST}`. The pre-composed `VE_<X>_HOST` vars stay ONLY for the worker's OWN vhost/DNS provisioning (where the host string itself is the subject), never as a URL a *different* component consumes.

## Broker/worker request shapes have ONE owner (issue #203)

The broker/worker client protocol — the six `/v1/cap/*` mint endpoints, the STS relay, worker put/get body types, audit append, the `knowledge:<ns>` service builder, the `0x`-omni normalizer — has ONE definition, split across two crates by transport-safety: the wire **types** live in [`agentkeys-protocol`](crates/agentkeys-protocol/) — pure serde, transport-free, compiles to `wasm32` — and the native **client** (cap-mint → STS → worker) in [`agentkeys-backend-client`](crates/agentkeys-backend-client/), which re-exports the types as `agentkeys_backend_client::protocol` (field types co-owned with [`agentkeys-types`](crates/agentkeys-types/)). The browser host [`agentkeys-web-core`](crates/agentkeys-web-core/) (wasm) depends on the SAME `agentkeys-protocol`, so the cap-mint body cannot drift across native vs browser — it used to (`ttl_seconds` required-`u64` vs `Option<u64>`, and web-core's copy was missing the #76 K10 PoP fields). web-core must NOT depend on `agentkeys-backend-client` directly: that crate pulls `aws-sdk-sts` + `tokio` + native `reqwest` (via the provisioner) and breaks the wasm build — the `wasm32` CI gate in `e2e-ci.yml` enforces this. **Never re-type a cap/worker body in a second Rust path or in bash** (the #200 drift-bug class). All Rust callers (the CLI's `BackendClient`, daemon `ui_bridge`, web-core) compile against the shared types, so a drifted shape is a compile error; bash and the web app are fixture-gated in CI.
...
- **Bash: never let a multibyte character directly follow an unbraced `$var`** — macOS **bash 3.2** (the operator laptop again) lexes variable names with single-byte ctype under UTF-8 locales, so `"$fn_id…"` parses as the unbound name `fn_id\xe2` and dies `fn_id�: unbound variable` under `set -u`, while bash ≥4 (CI + every Linux host) runs it fine — exactly how the #578 ship ceremony's seed leg died mid-run (`seed-base-images.sh`, 2026-08-12). The usual carriers are `…`/`—`/`–` in step/log strings. Brace it: `"${fn_id}…"`. CI-gated by [`scripts/utils/check-bash-unbraced-var-multibyte.sh`](scripts/utils/check-bash-unbraced-var-multibyte.sh) (e2e-ci `rust-checks`).
- **Bash: never put an `|| echo 0` fallback inside a `grep -c` capture** — `grep -c` PRINTS the count and EXITS 1 when it is zero, so `n="$(grep -c pat f || echo 0)"` captures `0`, a newline, and a second `0`; the next `[ "$n" -gt 1 ]` logs `integer expression expected` and reads as false, so the branch goes the right way by accident and the bug hides as log noise (`docker exec … grep -c` passes grep's exit status through — same trap). Assign first and fall back only on failure: `n="$(grep -c pat f)" || n=0`; inside a message string, `$(grep -c pat f || true)`. It logged from `host-common.sh`'s cargo-config guard in the VE broker converge (PR #724's `ve-e2e` run, 2026-09-23), and #728 re-introduced it in the dsh image smoke the next day. CI-gated by [`scripts/utils/check-bash-grep-count-fallback.sh`](scripts/utils/check-bash-grep-count-fallback.sh) (e2e-ci `rust-checks`).
...
- Crate names: agentkeys-types, agentkeys-core, agentkeys-cli, agentkeys-daemon, agentkeys-mock-server, agentkeys-provisioner
...
- **Always run clippy with CI's exact flags — `cargo clippy --workspace --all-targets -- -D warnings`.** A bare `cargo clippy` leaves warnings as warnings and exits 0, so local-green/CI-red is guaranteed the moment any lint fires; `-D warnings` is what the gate actually runs. It is NOT in the pre-commit hook (a full `--all-targets` clippy is minutes, too slow per commit) — which makes it a rule you have to remember. **The lint this catches most often is `dead_code` after a DELETION:** removing the last caller of a helper (or the tests that used it) leaves it unreferenced, and `-D warnings` turns that into a hard error in a file you never edited. After deleting code, re-run the full clippy line before pushing — `cargo build` and `cargo test` both stay green on dead code.
