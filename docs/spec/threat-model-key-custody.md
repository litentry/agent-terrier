...
- [`docs/spec/agent-channel-decoupling.md`](./agent-channel-decoupling.md) **§9 (channel threat-model deltas, #404 SHIPPED)** — the interaction-plane threat surface this key-custody doc does NOT cover: **prompt injection via channels** (kids' messages, camera scenes, any stranger-influenced content — mitigated by data-not-instructions, worker-stamped provenance + contact tier, deterministic outbound L2 gates, hooks/caps unchanged as the control plane); **gateway compromise** (blast radius = the bot credential + that transport's L2/L3, NOT authority — grants stay chain-verified); **contact impersonation** (a stolen phone — bounded by tiers; owner-tier never substitutes for operator auth); **family privacy** (contacts' messages are third-party PII — operator-only visibility, contact-zero-history D13, forward-only policy changes); **device subversion** (a flashed camera — per-device quotas + pub-only scope + atomic revoke); **device K10 at rest** (plain NVS on the ESP32-S3 today → flash-encryption is the tracked hardening; a lifted K10 is bounded to that one device's channel grants, never memory/cred); **template-tier mis-assignment** (advisory-only proposal, master confirms every reach diff).
