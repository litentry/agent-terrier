...
The runtime ask's push is the answerer's own (2026-09-24): an ungranted `tool:<class>` deny files a grant request through the daemon's `POST /v1/sandbox/self/propose` (the advertised propose verb's route) into the application's own `proposal:<ns>` queue — one ask per class per ten minutes, a stable key per class so a repeat refines the item instead of piling up, loud on the runtime log when it cannot land. (Until then the answerer spawned a shell helper at a path the image never had and swallowed the error — no owner ever saw an ask.) Once PR 2 of `plan/dsh-plugin-abstraction.md` compiles the grant view into the tool schema, an ungranted class cannot be attempted and this surface narrows to per-call approvals.
