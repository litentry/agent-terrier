...
- **Bridge HTTP contract** — route set, SSE frame shape, bearer semantics (`AGENTKEYS_BRIDGE_TOKEN`, fail-closed `AGENTKEYS_SANDBOX_MGMT_TOKEN`) unchanged. The bridge plugin binds `:8090` at plugin activation and creates the agent lazily (bind-first, the #589 readiness discipline).
