...
| OpenViking plugin | memory tools + auto-recall | resolves the server from `OPENVIKING_*` env with `http://127.0.0.1:1933` as its default (it never needs a file); the SERVER's `ov.conf` is planned by `agentkeys_memory_openviking::server_config` and rendered by the in-sandbox daemon at boot to `/opt/agentkeys/openviking/ov.conf` (2026-09-24 — `start-openviking.sh` only waits for the restore marker and execs); requires server support for the `viking://~` home alias (§7) |
...
Four surfaces, each backed by exactly one mechanism: **install sheet** (two sections — "Data & devices" for data-service grants, "Capabilities" for tool classes; one Touch ID = one #427 `executeBatch` minting every listed grant; `tool:schedule`, the one capability that acts with no one asking, is highlighted and opens onto the template's scheduled tasks — when, in household time, and the exact instruction each run is given — before the Touch ID, and the same line on the app page afterwards, owner ask 2026-09-24; the sheet renders data only — which line is highlighted and what every capability line says come from the protocol's capability catalog, each task's time from the template catalog's derived `when`, arch §22f F2), **permissions page** (the same two sections as toggles — a toggle *is* a grant; off = the revocation ceremony; "Built with" is read-only disclosure), **runtime ask** (a companion push via the #573 inbox: Allow once / Deny; "Always allow" opens the grant ceremony; ignoring it fails closed), **activity report** (per app, in owner language, denials shown). No dsh UI is mounted; parent-control is the only owner-facing surface.
