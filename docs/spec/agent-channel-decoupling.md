...
- **The parent-control React UI** — channel/contact management panels + the feed-history browser (the backend read contract + the D13-safe view shipped in #410; the frontend is the follow-up). *Update (2026-07-11): the contact half landed in #418/#419 (gateway connect + family binding), and the device half landed with the #404 UX-decoupling PR — the pairing page is delegates-only, the channels page pairs devices (claim-with-channel-attachment → `is_device` accept card hard-enforcing §14.10 → bound-device grid → channel-grant edit · Touch ID), and the headless twins shipped (`agentkeys agent accept`, the D9 gate on `/v1/agent/resolve` via `is_device`, channel-e2e step 16). Still open: the **feed-history browser** (D13 operator-only feed view) and channel-name persistence across daemon restarts.*
