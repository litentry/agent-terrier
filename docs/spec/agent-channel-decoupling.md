...
| arch.md site | Change |
|---|---|
| §5 canonical names | add `channel`, `channel adapter`, `channel feed`, `contact gate`, `contact`, `device` (= channel endpoint), `default delegate`, `context-sub/pub:<ns>` rows (the §3 table); record the §17.6 rename alias + the frozen `memory:`/`inbox:` wire spellings |
