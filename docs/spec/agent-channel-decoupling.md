# Agent ↔ device decoupling — channels, contact gates, contacts
...
| D11 | **Direction vocabulary is UNIVERSAL** | rename the shipped `memory:<ns>`/`inbox:<ns>` wire ids now | **pub/sub is the direction vocabulary for channels** (`channel-pub/sub:<id>`). For the master↔delegate context flows the 2026-07 compromise (prose `context-sub/pub`, wire frozen) was **superseded on 2026-09-13**: the wire was renamed as a clean break to **`knowledge:<ns>`** (pull / read) and **`proposal:<ns>`** (propose), the grant strings are the names, and the git vocabulary (origin · clone · pull · proposal · merge · commit) is the UI vocabulary — arch.md §5, `docs/plan/knowledge-repository.md` | the #339 read/write split-by-service-id |
...
| **AI runtime** | external software (not our identity) | — | equipped *by* a delegate | the external AI app a delegate wraps (dsh, Claude Code, xiaozhi) |
...
- **Externally-authenticated principals** — contacts (the family) — are authenticated *for* them by a channel's transport, hold no keys and no caps, and are authorized by L3 policy at the contact gate. "Keyless" is the key-custody fact; "externally authenticated" is what actually happens.
- **Resources** (channels) and **infra** (broker / worker / contact gate / adapters) are **not principals** — they carry the flows and enforce policy, but authority is only ever the master's on-chain, master-signed grant.
...
| `channel adapter` | *what bridges this transport to canonical channel events?* | the transport-specific live component: a contact gate (weixin), the SES routing Lambda (email), a firmware task (mic/display/touch), the delegate's sandbox bridge (chat sessions, UI feeds) | per-kind worker crates / firmware modules |
...
| `contact gate` | ***how capably can many externally-authenticated humans interact with agents through a channel?*** | the capability boundary for humans-without-keys: authenticates each contact via the transport (openid today; voice-id / app session later), enforces L2 (how the agent may behave) + L3 (what this contact may ask), stamps provenance, routes. Where the transport identity is scarce (one WeChat bot per KYC'd account), the contact gate also custodies that ONE credential. **A PEP, never an authority** — grants stay master-signed + chain-verified | `agentkeys-worker-channel-weixin` (etc.); named `gateway` until 2026-09-09 (arch.md §5 canonical names) — the code identifiers and the `/v1/gateway/*` prefixes keep the old word until #684 |
...
| `knowledge:<ns>` / `proposal:<ns>` | *in which direction may a delegate touch the master's canonical context?* | the #339 grants by their wire names (renamed 2026-09-13 from `memory:`/`inbox:`, no compatibility window): `knowledge:<ns>` = pull the canonical namespace (distribution), `proposal:<ns>` = propose into the inbox (absorption) — the **curation-gated** context substrate (D12). Their direction vocabulary is git's (pull / proposal), not pub/sub — pub/sub stays the CHANNEL vocabulary. | **retired:** `context-sub:<ns>` / `context-pub:<ns>` (2026-07 prose), `memory:<ns>` / `inbox:<ns>` (pre-2026-09-13 wire) |

**Naming collision — DECIDED (§14.2), amended 2026-09-13:** the new concept takes the bare word **`channel`**; arch.md §17.6's "the two channels" (distribution/absorption) are retitled **context flows**. Their grants were first given the pub/sub prose names `context-sub/pub:<ns>` (D11, 2026-07) and then, on 2026-09-13, renamed on the wire itself to `knowledge:<ns>` / `proposal:<ns>` with the git vocabulary (pull / proposal / merge); the §5 alias rows record every retired spelling.
...
ChannelEvent {
  ...
  producer:  actor_omni (keyed)  XOR  contact_id + contact_tier (externally-authenticated, contact-gate-verified),
...
INBOUND (contact → agent):
  contact ──weixin──▶ CONTACT GATE              ──▶ channel feed ──▶ delegate
...
OUTBOUND (agent → contact/device):
  delegate ──channel-pub cap──▶ channel worker / contact gate ──L2 check──▶ transport ──▶ contact / display
...
1. **Four-layer isolation (§17.5)** applies verbatim: broker cap-mint checks the `channel-pub/sub:<id>` grant; the channel worker independently re-verifies on-chain; IAM PrincipalTag + the new per-data-class `channel` bucket bound the blast radius. Stage-3 grows the matching negatives (cross-agent channel access → denied at mint; wrong direction → denied; contact not in registry → dropped at contact gate).
2. **No credential ever reaches a delegate**: not the WeChat bot secret (contact gate custody, #384 pattern), not S3 creds for operator-owned feeds (A′ server-side-STS pattern, #295 P1).
...
| 7 | **default-delegate sandbox spawn** (onboarding path only) | cold veFaaS spawn + image + runtime boot | `plan/spawn-sandbox-on-pair.md` (operator-internal), #396 in flight |
...
In one line: **keyed actors (delegates, devices) are authorized by L1 grants they prove with K10 and shaped by L2 policy; contacts are authorized by L3 policy the contact gate enforces for them; channels themselves carry only defaults.**
...
|---|---|---|---|---|
| **weixin** | **two drivers, one PEP (decided 2026-07-09):** iLink personal bot (`ilink` — experiment) / official account (`oa` — production) | ❌ one per KYC'd account either way; re-binding overwrites; `oa`: user-initiated reply windows (~48 h) + content review; `ilink`: DM-only, per-message `context_token` reply authorization | **shared contact gate**; operator-owned feed; contacts authn'd by sender id (openid / ilink user id); `oa` replies ride the reply window — proactive pushes need template-message rights (L2 `initiate` right models this); `ilink` replies echo the stored `context_token` | the v1 priority channel |
| **telegram** | BotFather bots | ⚠️ bots are mintable, but the owning account is phone-KYC'd; bots cannot message a user first | contact gate (shared account custody) even though bots could be per-agent; first-contact = user starts the bot (a natural contact-bind ceremony) | |
...
2. **Contact bind ceremony** (mirrors pairing's claim shape): master invites a contact from parent-control → one-time bind code → the human sends it to the bot → contact gate records `openid → pending` → master approves; the small model **proposes** the tier + reach from the household template (D5), the master **confirms**. No keys, no chain write — the registry is a master-authored `policy`-class doc.
...
4. **Delivery**: stamped event → the target agent's subscribed feed → wake-on-event if hibernated → the agent's reply comes back as a channel-pub the contact gate L2-checks (reply window, counterparty) and sends.

## 8. Worked examples (the brief's three)

- **Chef.** Family members text the bot all day ("two cookies", "30 min swim"); contact gate authenticates each contact, stamps provenance, appends to the chef's subscribed `family-weixin` feed. At 16:00 a scheduled job wakes the chef: it consumes the day's events, computes the dinner energy budget, `channel-pub`s the menu to the `kitchen-display` feed, and sends the helper a summary (L2: helper is an allowlisted counterparty; initiate right granted for that one contact).
- **Doorkeeper.** The front-door camera (device, pub-only) publishes motion frames. The doorkeeper delegate (sandbox, wake-on-event) consumes, classifies, writes "kids home 15:42" into its `knowledge:<ns>`, and pubs a notification event that the contact gate delivers to `partner`/`elder`-tier contacts. When a parent later asks "did the kids come back?", L3 admits the question, the router picks the doorkeeper, and it **answers from its own memory — contacts never browse the camera feed itself; the feed history is operator-only, in parent-control (D13)**.
- **Weekly spend stats.** The owner asks the bot for weekly agent costs. The contact gate maps openid → `owner` tier, but operator-grade data still requires operator-grade auth: v1 answers with a deep link into parent-control (session-authenticated); a later option is a steward delegate explicitly granted a metering-read service over the #384 per-user usage rollups. Kids asking the same question are refused at L3.
...
2. **Contact gate compromise** — blast radius = the bot credential + L2/L3 enforcement for that transport, **not authority**: it cannot mint grants (chain-verified at workers), cannot read feeds outside its channel role, cannot forge provenance *silently* (audit rows anchor). Same bounded-PEP posture as a compromised worker in §17.5.
3. **Contact impersonation** — platform-identity theft (a stolen phone) impersonates a contact. Tiers bound the damage (guest/kid tiers can't reach sensitive agents); owner-tier never substitutes for operator auth (L3 rule); per-contact anomaly quotas at the contact gate.
...
| the #339 direction split (`knowledge:<ns>` / `proposal:<ns>`, renamed 2026-09-13) | the per-direction service-id pattern `channel-pub/sub:<id>` generalizes (D11) — same shape, different substrate (D12) |
...
| `agentkeys-gate` #384 | the **model gate**: scarce-shared-credential custody + per-user metering — the gate pattern's first instance; the contact gate (the retired "gateway") is its second |
...
Net-new inventions are exactly three: the **ChannelEvent envelope + feeds**, the **contact registry + L3**, and the **contact gates**.
...
| 2 | **WeChat contact gate MVP** (the priority — the main human interface) | bot custody + contact registry + household-template bootstrap + bind ceremony (model-proposed tier, master-confirmed) + L3 + `/alias` routing (deterministic only) + reply-window L2; targets the delegates that already run today | 1 |
...
- **Phase 2 WeChat contact gate (#407)** — `agentkeys-worker-channel-weixin`: #384 bot-credential custody, the `contact` registry with household tiers, the L3 PEP, `/alias` routing, GatewayRelay/ContactBind op_kinds, D13 contact-zero-history; mock-driver e2e + the operator setup doc.
...
- **Phase 6 promotion (#411)** — arch.md §5/§15/§17.5/§22e canonical-name + endpoint + status rows, this record, the user-manual channel / contact / contact gate / device entries, the threat-model deltas (§9).
...
- **Camera/display/`session` firmware paths + a bound-device feed round-trip** — no camera on the `esp32s3-touch-lcd-4b`; firmware doesn't build in CI. The device-core FFI enabler ships; the firmware wiring is the release-cycle gate. *Update (2026-07-13, #423): the bound-device feed round-trip landed HEADLESSLY — a software-K10 kitchen-display device actor is claimed (HDKD child omni), bound on-chain (`registerAgentDevice(TIER_DEVICE)` + enumerated `channel-pub/sub:<id>` `setScope`, one sponsored UserOp via the headless accept), mints channel caps with its OWN child-omni session (channel ops joined the `CanonicalFetch`/`Append` actor-session set — arch.md §17.5), and duplex round-trips real feeds with deny-by-absence/wrong-direction/memory/cross-actor negatives (`channel-e2e-demo.sh` steps 17-19). Only the FIRMWARE leg (the same flow signed by `net/device_identity.c` on hardware) remains release-cycle-gated.*
...
- **The parent-control React UI** — channel/contact management panels + the feed-history browser (the backend read contract + the D13-safe view shipped in #410; the frontend is the follow-up). *Update (2026-07-11): the contact half landed in #418/#419 (contact gate connect + family binding), and the device half landed with the #404 UX-decoupling PR — the pairing page is delegates-only, the channels page pairs devices (claim-with-channel-attachment → `is_device` accept card hard-enforcing §14.10 → bound-device grid → channel-grant edit · Touch ID), and the headless twins shipped (`agentkeys agent accept`, the D9 gate on `/v1/agent/resolve` via `is_device`, channel-e2e step 16). Still open: the **feed-history browser** (D13 operator-only feed view) and channel-name persistence across daemon restarts.*
...
| arch.md site | Change |
|---|---|
| §5 canonical names | add `channel`, `channel adapter`, `channel feed`, `contact gate`, `contact`, `device` (= channel endpoint), `default delegate`, `context-sub/pub:<ns>` rows (the §3 table); record the §17.6 rename alias + the frozen `knowledge:`/`proposal:` wire spellings |
...
| §22d | contact gate = PEP placement note; router + tier-proposer advisory-only rule |
...
- **Per-agent Telegram bots** (mintable path skipping the contact gate) — possible later; start shared for one custody model.
- **The D11 wire rename — done differently, 2026-09-13:** `memory:`/`inbox:` were renamed to `knowledge:<ns>` / `proposal:<ns>` as a clean break (no users, no compatibility window) instead of the planned `context-sub/pub:<ns>`; the `agent→delegate` on-chain selector rename still waits for a contract cutover.
