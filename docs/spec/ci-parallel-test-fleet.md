...
1. A dependency-free `slot-claim` job starts with the run (seconds in — not after the ~10-min `rust-checks`, which would make claims invisible exactly when concurrent runs balance) and publishes its claim as the job annotation `slot-claim slot=N group=…` — a stable machine-readable contract read by other runs' claim steps and by `scripts/fleet-status.sh`.
...
`bash scripts/fleet-status.sh [--watch [secs]]` is the read-only operator view of the fleet (issue [#279](https://github.com/litentry/agentKeys/issues/279) v1, CLI surface — no always-on service, runs on the operator laptop with existing `aws`/`gh` creds). Three sections:
