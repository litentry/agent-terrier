...
2. It counts the slots claimed by every other queued/in-progress `e2e-ci.yml` run (a run from a pre-balancing branch counts as slot 1 once started — its workflow hardwires the slot-1 instance + deployer; a queued one holds nothing yet), then picks the least-loaded slot. Ties go to the static hash (stable spread on an idle fleet), then the lowest slot.
