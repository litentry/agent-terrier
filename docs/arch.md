...
| `context system` | **The general agent-context substrate** — one curation-gated lifecycle (delegate working copy → `inbox:<ns>` push → master curates → canonical → distribute via pull / at-spawn, §17.6) carrying **typed** context: `knowledge` (recalled facts), `skills` (registered capabilities), `persona` (applied framing docs, e.g. `SOUL.md`), `resources`. What varies per type is the **adoption-gate strictness + runtime application, never the machinery** (invariant: nothing enters canonical except through the master's curation gate). Matches OpenViking's model (memories/resources/skills = one `viking://` context tree). **Code + wire spellings stay frozen**: `DataClass::Memory`, `memory:<ns>` / `inbox:<ns>` services, `$MEMORY_BUCKET` (a `context:` prefix rename is a deferred major-version cutover). Design: `plan/master-hub-topology.md` §16 (operator-internal), #390. | "memory system", "shared memory", "master-hub memory" (pre-#390 names for the same system); `DataClass::Memory` / `memory:<ns>` / `inbox:<ns>` (code / wire — frozen). |
| `policy` (data class) | **The control-plane data class** (#201): the operator's NL policy, category taxonomy, and compiled grant view — **access-control on the access-control**, master-only, **never agent-loadable** (the governed delegate holds no cap for it; §17.3). Canonical prose is **policy data class** — the word "config" formerly named BOTH this and agent-shaping documents (`SOUL.md` / skills / `AGENTS.md`), which are **context** (`context system` row) and NOT this class; that collision is retired. **Code + infra spellings stay frozen** (S3 buckets can't be renamed; a code rename is a deferred major-version cutover): `DataClass::Config`, `agentkeys-worker-config`, `$CONFIG_BUCKET`, `/v1/cap/config-{store,fetch}`, `config/<svc>.enc`, `AGENTKEYS_WORKER_CONFIG_URL`. | `config` / `Config` (code / infra — frozen aliases); "permission config", "policy config" (§17.3 prose). |
...
- **Classifier-service — write-side dual of the engine (§15.6; substrate landed #207 items 2-3-6):** the engine ranks at *read*; the **classifier-service** compiles natural-language intent → the structured policy attribute the gate enforces (memory→namespace, creds→service-category, IoT→device-tier), and tags novel requests — *NL-programmable, deterministically-enforced* authorization fleet-wide, with no model on the gate's hot path. **Landed:** the `agentkeys-worker-classify` crate (a **COMPUTE** gate — same cap + chain-verify chain as the storage workers via `agentkeys_worker_creds::verify`, but NO S3 bucket/KEK; port 9097, `classify.<zone>`), `CapOp::Classify` + `/v1/cap/classify` (data-class-bound), and the bundled **category catalog** (`catalog.rs` — entity→category + sensitivity floor + signed vendor overlays). The classify engine is the deterministic **tier-0** (catalog lookup + a small keyword-rules COMPILE); the LLM-backed engine (`engine:"llm"`, the open-vocabulary tail) is the deferred enhancement (no model on the gate, ever). **Its data + eval foundation landed (#322):** the `PolicyIntent` request-parse profile + the deterministic output validator (`agentkeys-catalog` `policy_intent.rs` / `validate.rs` — shared with the worker so the dataset contract == the runtime contract; floor-lowering, decision fields, and fabricated operations are rejected), the `agentkeys-classify-dataset` validate/dump CLI, a catalog-grounded seed + held-out gold corpus, and a validator-gated LLM generation harness + safety-focused eval (`ml/policy-intent-dataset/`). The model itself (fine-tune + shadow-mode) is the follow-up. Design: `plan/policy-intent-dataset.md`. **Daemon bridge + auto-distribution (#207 items 5/7/8, landed):** the catalog is a shared `agentkeys-catalog` crate the daemon also uses; the daemon's `--classify-url` bridge (mint a master-self `Classify` cap → worker TAG, with the local catalog tier-0 as fallback) backs `POST /v1/master/classify/tag` (cred auto-categorize) + `POST /v1/master/classify/propose` (classify an agent's surface → scopes tiered `auto`/`k11` by the catalog sensitivity floor). `propose` writes no scope — only the K11-gated grant path does, so an unconfirmed sensitive category never becomes a grant (§3 invariant). Design + three-phase plan + caching/efficiency model: `plan/classifier-service.md` (operator-internal).
...
asserted per-data-class in `e2e/suite-3-isolation.sh` steps 11-12.
...
bash scripts/operator/chain/heima-k3-rotate.sh
```
...
|---|---|---|---|
| 1. Broker cap-mint | session JWT's omni == request's operator_omni; device-binding; ROLE_CAP_MINT; service in scope | `handlers/cap.rs` | `e2e/suite-3-isolation.sh` step 13 |
...
**Fifth control — the PROVISIONING identity is scoped too (#372, both clouds):** the four layers gate the *runtime* path; #372 scopes the *operator/admin* axis. The AWS admin group and the VE bootstrap admin no longer carry `AmazonS3FullAccess` / `TOSFullAccess` — they hold mirrored custom policies (single source of truth: [`scripts/operator/policies/`](../scripts/operator/policies/); Sid-level cross-cloud parity CI-gated by [`scripts/utils/check-storage-policy-parity.sh`](../scripts/utils/check-storage-policy-parity.sh)) granting bucket lifecycle management but **zero object access on the vault/memory/config data buckets** — a stolen operator credential can no longer read ciphertext or delete/overwrite data-class blobs. The VE broker *signing* identity is narrower still: STS mint only, zero storage actions. Applied idempotently by `setup-cloud.sh` step 16 / `setup-cloud-ve.sh` steps 11 + 56; stage-3 step 24 carries the negative (admin `GetObject` on a data bucket → `AccessDenied`). Side-by-side table: [`docs/spec/ve-broker-runtime-port.md`](spec/ve-broker-runtime-port.md) "Storage-plane provisioning identities".
...
**Generalization — the two channels carry typed *context*, not just recall content (the `context system`, §5; [#390](https://github.com/litentry/agentKeys/issues/390)).** The same lifecycle distributes every kind of agent-shaping context — `knowledge` (recalled facts, the original "memory"), `skills` (registered capabilities — delegate-producible, e.g. refined on one machine and adopted fleet-wide through the same push→curate→distribute loop), `persona` (applied framing docs like `SOUL.md`), `resources`. **What varies per type is the adoption-gate strictness and the runtime application, never the machinery**: knowledge = light curation, injected per turn; skills = strict diff-review curation (near-executable ⇒ higher injection/behavior risk), delivered as files; persona = master-authored-only today + edit-time validation (the agent-agnostic guardrail) + a locked always-appended base layer, applied fresh each turn. The surviving invariant: **nothing enters canonical except through the master's curation gate — direction is a per-type gate-policy outcome, never a hard-wired class property.** The **`policy` data class (§5, #201) stays outside this system**: it is the control plane's own data (access-control on the access-control), master-only and never agent-loadable — the former "config" word covered both it and agent-shaping docs, and that collision is retired. No chain change: the namespace encodes the type (`skills` is a namespace), so the existing `memory:<ns>` / `inbox:<ns>` grants cover typed context. **Landed (#390):** the wire `kind` (`agentkeys-protocol::ContextKind`, absent = `knowledge`; worker-stamped on inbox-append), the per-kind curate gate in the daemon (persona → `persona_not_inbox_adoptable`; skill → viewed-body `confirm_content_hash` watermark + size cap; mirrored in the parent-control inbox + the CLI `inbox-accept`), and the **persona editor**: `/v1/master/persona` {get, edit, rollback, delete} stores versioned `soul:<delegate-omni>` docs in the **reserved master-self `persona` namespace** (never granted to a delegate, never in the recall taxonomy, the plant rejects it — the persona module is its single writer), validates at edit time (32 KiB cap, secret-shape scan, the agent-agnostic guardrail), and applies live into the bound sandbox via the bridge's `/v1/context/apply` + `/v1/agent/restart` (daemon `--sandbox-bridge-url` / `AGENTKEYS_SANDBOX_BRIDGE_URL`; the sandbox `AGENTS.md` is composed = owner section + the locked `agent-terrier.md` always appended). Store-only vs live-applied is always explicit in the response (`applied` + detail). Deferred: per-delegate sandbox routing (spawn-on-pair), skill file-delivery, the #322 classifier advisory, `resource`. Design: `plan/master-hub-topology.md` §16 (operator-internal).
...
- **Why staged, not default-on:** agents already register `device_key_hash =
  ...
  (`e2e/scripts/heima-register-master-k10.sh`, `registerAdditionalMasterDevice`,
...
- `scripts/operator/chain/heima-device-register.sh` — empty attestation + empty K11 assertion on first call (§22b.1, §22b.3). **DEPRECATED escape-only** (old-model EOA wrapper, no orchestrator callers): the supported register is the #164 passkey-account path (`register_first_master` → `e2e/scripts/erc4337-register-master.sh`).
- `scripts/operator/chain/heima-agent-create.sh` — empty K11 / link-code-redemption stubs (§22b.3).
- `scripts/operator/chain/heima-scope-set.sh` — stub K11 assertion (§22b.1).
...
> **Disambiguation.** This dropped seam is the *IAM-guarantee* proxy only. It is unrelated to (a) the daemon's **localhost credential proxy** (§12 — the sidecar that injects minted credentials at a Unix socket; shipped, unaffected), (b) the **"broker-not-proxy" principle** (we mint credentials, never proxy per-operation reads/writes), and (c) the **#384 LLM-egress relay** (§22d.3a — key custody + metering for our OWN managed runtimes; not an IAM-guarantee seam). All three keep the word "proxy"/"relay" and are untouched.

### 22d.3a The LLM-egress relay (#384) — one implementation, two placements

The #384 relay (`agentkeys-gate`, GateTurn 90) exists for **key custody + per-user metering** (#332/#338), not authorization — control stays in hooks + caps (§22d.2); the relay grants nothing. It inserts through the one config knob every OpenAI-compatible agent runtime already has — `base_url` + `api_key` — carrying a per-sandbox / per-delegate **relay key**; the vendor key never enters the agent's environment, so a prompt-injected env/shell dump yields only a budgeted, attributable, revocable relay key.

**Placement is per credential family, chosen by host ownership (decided 2026-07-03):**

- **Operator-run sandboxes → the central gate service (NOW).** Vendor keys never leave operator hosts; exact per-call metering with pre-burn budget 429s. Throughput note: the relay is a thin SSE proxy (KB/s per conversation) — the scaling concern is availability (one box today), not compute.
- **User-owned delegates (Mac-mini class), TEXT/agent traffic → daemon-local relay (LONG RUN).** The daemon spawns the *same relay binary* on `127.0.0.1`; it holds only **broker-minted, TTL'd, endpoint-scoped ARK temp keys** (Q17/#337). A local relay hides keys from the **agent**, never from the **owner** — owner risk is bounded by TTL × scope, and usage is enforced at **mint time**: the broker refuses re-mint over budget (worst case = one TTL window of unmetered burn, detected by gate-metered-vs-vendor-console divergence). Pooled long-lived keys on user hardware are forbidden. Scoped to text/agent turns because those originate locally — a local relay is the shortest path for them.
- **Voice turns are NEVER split across placements (decided 2026-07-03).** A split voice pipeline (e.g. speech relayed centrally while the LLM leg runs delegate-local) zigzags one turn across hosts — audio up, text down, reply text up, audio down — two extra WAN legs inside the ~1 s voice budget. A voice turn runs **in one place end-to-end**: **today**, the central gate host (ASR → LLM → TTS chained on-box, one device↔gate streaming round trip, every leg metered); **end-state**, all-vendor-side — RTC carries transport + ASR + TTS and calls our turn endpoint (§22d.3), or the end-to-end realtime speech model consumes audio tokens directly (no separate ASR/TTS stages at all) — and our box leaves the audio path entirely. Speech tokens exist only on operator hosts, isolated per family (#338, `agentkeys-inference-creds`); the speech least-privilege / temp-token path is tracked in [#386](https://github.com/litentry/agentKeys/issues/386).
- **BYOK delegates**: with the user's *own* vendor key the relay is optional — run the local relay for visibility, or put the key straight into the agent's config with no relay at all. Their key, their blast radius, their bill; pooled-key metering/budgets don't apply, and the control plane (hooks + caps) is unchanged either way.

**This is not the dropped generic proxy (above).** That was an open `OPENAI_BASE_URL` gateway for *unmanaged third-party* hosts, as an IAM-guarantee substitute. The relay's callers are **our own managed runtimes** (spawned sandboxes, paired delegates) and certified-stack turns; it keeps the "no retry, no fallback, no caching, no orchestration" discipline, and dropping it would change cost attribution, not authority.
