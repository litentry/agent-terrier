...
| `SandboxTeardown` | 54 | `{device_key_hash, sandbox_id, reason}` | broker sandbox lifecycle (#377 — `reason: "unpair"` on a confirmed `revokeAgentDevice`; `reason: "update"` on a #577 in-place image update, always paired with the replacement's `SandboxSpawn`; veFaaS timeout expiry is not broker-observed) |
