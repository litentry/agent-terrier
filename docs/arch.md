...
- **A delegate's OWN-namespace op mints its credential server-side too (#716):** a `/v1/memory/put` or `/get` that carries no relayed `X-Aws-*` creds but the delegate's session bearer and a delegate cap (`operator ≠ actor`) makes the worker redeem that cap at the broker's **`POST /v1/cap/own-sts`** — ACTOR-tagged STS scoped to that one service's objects under `bots/<actor>/memory/` (`Store` = put + the D-K5 compare-and-swap read; `Fetch` = read) — never the worker's default client. This is the VE credential path the #594/#694 sandbox checkpoint never had (measured 2026-09-22: every checkpoint put 502'd `no providers in chain provided credentials`); on AWS it replaces the silent instance-profile ride. The delegate never holds cloud creds; a master-self op keeps the client relay. Regression gate: suite-3 step 26.
