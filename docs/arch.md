...
| `agentkeys-gate` | operator host | the **model gate** — the door OUT to the model provider: custodies the ONE LLM key, meters every turn (#384); the gate pattern's first instance (§5 canonical names, `gate`); never control |
...
| `AgentKeys plugin suite` | **LIVE (#611/#612/#613/#653/#662):** the `@agentkeys/*` dsh plugins that enforce AgentKeys authority inside a delegate's AI runtime — monotonic tool guard, approval answerer, vault/cap-backed credential provider, gate LLM route, audit tee, bridge plugin, and the `publish_to_slot` act-verb tool (2026-09-17: a delegate publishes to its granted feeds without a shell — guard-allowed on any `channel-pub:` grant, the feed itself decided at cap-mint) ([`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md) §1, §4). | "bundle" (dsh's own distribution format), "hooks" (the retired #560 delivery) |
| `capability service` | **LIVE (#662 compiler · #611 guard):** a scope-grant string consumed only by the app-template compiler / tool guard — `plugin:<id>` (may a capability provider mount), `tool:<class>` (may an action family run) — never cap-minted by a worker; makes the in-loop action plane a pure projection of on-chain grants ([`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md) §4.2). | a data class; `knowledge:`/`cred:`/`channel:` (those are data services) |
