...
| Memory engine (#147/#566) | OpenViking as Hermes' native provider (in-sandbox, gate-bounded corpus via the daemon mirror); engine disabled-but-clean without an embed config | `MemoryEngine` trait behind the mirror; Hermes side swaps via `memory.provider`; #183 profile adapter = the N-vendor endgame |
