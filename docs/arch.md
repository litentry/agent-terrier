...
| `capability service` | **LIVE (#662 compiler · #611 guard):** a scope-grant string consumed only by the app-template compiler / tool guard — `plugin:<id>` (may a capability provider mount), `tool:<class>` (may an action family run) — never cap-minted by a worker; makes the in-loop action plane a pure projection of on-chain grants ([`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md) §4.2). | a data class; `memory:`/`cred:`/`channel:` (those are data services) |
...
| curated resource items (kind, tags, sensitivity, version, content hash) | config: **`resource-registry`** (#666) — written by the master's curation; the item's bytes live in canonical memory (`memory:<ns>`, `resource` kind) |
...
**The four verbs** (the runtime contract at its smallest): **perceive** (events from every bound slot arrive as turns — text, `image`, `audio-clip`, `command`; a media event runs a per-kind pre-turn from the template's `skills/perception.md`, originals read by reference, never downscaled), **act** (publish `text` / `doc` / `image` to any bound `pub` slot), **remember** (keyed objects under `memory:app-<label>`, proposals via `inbox:app-<label>`), **may** (every action is a projection of the on-chain grant set — the broker at cap-mint, the worker at use, the in-loop guard in front of tools). Conformance cases R1–R8 in the plan; the suite is `e2e/suite-8-apps-framework.sh`.
