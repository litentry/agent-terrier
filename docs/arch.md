...
| `vault/memory/config/audit/email/payment_audit bucket` | One S3 bucket per data class (§17); per-actor prefix `bots/<actor_omni_hex>/` (config per-operator, master-only). | `$VAULT_BUCKET` … |
...
**Reserved (defined in the channel spec, promoted at its phase 1):** `channel`, `channel adapter`, `channel feed`, `gateway`, `contact`, `device` (= channel endpoint), `default delegate`, `channel-pub:<id>` / `channel-sub:<id>` — one-question-per-term definitions in [`spec/agent-channel-decoupling.md`](spec/agent-channel-decoupling.md) §3.
...
Unclaimed bytes in each 10-block + `100-255` are reserved — the device family claimed `53`/`54` for the sandbox lifecycle per #377, so `55-59` (not `53-59`) is what remains free there; channel-family kinds land with the §22e plan's phase 1. `GateTurn` attribution: envelope omnis both carry the OWNING USER; device/api-key are body-level rollup dimensions (`GET /v1/usage`).
...
Workers reject mismatched classes with 403 `cap_data_class_mismatch` — the cap-layer twin of the IAM cross-bucket gate.
...
- **22d.4:** neither seam makes AgentKeys a Task Host or a generic gateway (strategy §2.4 line). The **channel gateway (§22e) is likewise a PEP, never an authority**; the routing/tier classifiers stay advisory.

## 22e. Channels — agent ↔ device decoupling (DESIGN)

> **Canonical design spec: [`spec/agent-channel-decoupling.md`](spec/agent-channel-decoupling.md)** (the definition study it draws on — `research/bytedance-agent-channel-model.md` — is operator-internal). Status: design; owner decisions resolved 2026-07-07/08 (spec §14). This section is the arch-level summary; the spec's §12 promotion map amends §5/§10.2/§15/§17.5/§17.6/§22c.4/§22d as phases land. Diagrams: [`assets/agent-device-decoupling-topology.svg`](assets/agent-device-decoupling-topology.svg) (topology) · [`assets/device-bind-sandbox-spawn-security.svg`](assets/device-bind-sandbox-spawn-security.svg) (bind/spawn key-custody).
