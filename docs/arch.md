...
| `AI runtime` | **The external AI application** a delegate equips — never our identity. Canonical is **AI runtime**, not bare "runtime". **Live today: Hermes. Target (owner decision 2026-08-23, not yet live): DeepSeek Harness (`dsh`) with the AgentKeys plugin suite — [`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md); Hermes is fully deprecated at the end of that migration (`plan/dsh-runtime-migration.md`, operator-internal).** | Claude Code, Hermes, xiaozhi; avoid bare "agent"/"runtime" |
...
- **Engine seam (Position C, re-shaped by #566):** the worker stays **store + gate only**. OpenViking is Hermes' **first-class native memory provider** (`memory.provider: openviking`, in-sandbox engine on :1933), and the gate bound moved to **ingest-time**: the daemon's **distribution mirror** (`agentkeys-daemon/src/memory_mirror.rs`) is the only writer of canonical-derived content — it probes each candidate namespace via cap-mint → `canonical-get`, mirrors what the worker authorizes, and delete-throughs what it refuses (403 = revoked ⇒ removed at the next pass; respawn rebuilds from canonical). Engine corpus ⊆ delegate working memory ∪ gate-authorized slices, structurally per-sandbox. Never load-bearing (engine down ⇒ Hermes built-in memory). Plan: `plan/issue-566-openviking-native-memory-provider.md`; decision record: `research/memory-build-vs-gate-decision.md` (operator-internal); user explainer: [`wiki/memory-providers-and-agents.md`](wiki/memory-providers-and-agents.md). (The pre-#566 `pre_llm_call`-hook delivery was retired with the MCP crates, #560.)
...
| Memory engine (#147/#566) | OpenViking as Hermes' native provider (in-sandbox, gate-bounded corpus via the daemon mirror); engine disabled-but-clean without an embed MODEL — the embed key/base default onto the gate's metered `/v1/embeddings` relay (#572, no raw vendor key in-sandbox) | `MemoryEngine` trait behind the mirror; Hermes side swaps via `memory.provider`; #183 profile adapter = the N-vendor endgame |
