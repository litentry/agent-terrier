...
| `bridge` | **The in-sandbox seam between the daemon and the AI runtime** (#577/#611: `packages/agentkeys-dsh` — session export, context apply, jobs, restart; reached at `sandbox_bridge_url` with the derived bridge token). Only this is "the bridge". The daemon's local HTTP API the console talks to (`:3114`) is the **daemon API** — its code module keeps the name `ui_bridge` as an identifier only. | "dsh bridge", "sandbox bridge", `ui_bridge` (module name) |
