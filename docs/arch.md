...
| `AgentKeys plugin suite` | **TARGET (not live):** the `@agentkeys/*` dsh plugins that enforce AgentKeys authority inside a delegate's AI runtime — monotonic tool guard, approval answerer, vault/cap-backed credential provider, gate LLM route, audit tee, bridge plugin ([`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md) §1, §4). | "bundle" (dsh's own distribution format), "hooks" (the retired #560 delivery) |
| `capability service` | **TARGET (not live):** a scope-grant string consumed only by the runtime's preset compiler / tool guard — `plugin:<id>` (may a capability provider mount), `tool:<class>` (may an action family run) — never cap-minted by a worker; makes the in-loop action plane a pure projection of on-chain grants ([`spec/delegate-runtime-dsh.md`](spec/delegate-runtime-dsh.md) §4.2). | a data class; `memory:`/`cred:`/`channel:` (those are data services) |
