...
Agents don't fetch memory over the network from your store — the engine runs **next to the agent** (in its sandbox), holding a local, gate-bounded copy of just the authorized slice, rebuilt automatically whenever the sandbox restarts. Searching it is a local call (the agent's own memory tools), so it's fast, and "rummaging" can never reach beyond what you granted — content outside the agent's grants is simply not in its engine. If the engine is down or not enabled, the agent falls back to its built-in notes and keeps working.
